...
impl Shared {
    ...
    pub(crate) fn on_dial_failure(&mut self, peer: PeerId, reason: String) {
        ...
        while let Ok(new_stream) = receiver.try_recv() {
            let _ = new_stream
                .sender
                .send(Err(OpenStreamError::Io(io::Error::new(
    ...
    pub(crate) fn sender(&mut self, peer: PeerId) -> mpsc::Sender<NewStream> {
        // Upstream uses `rand::seq::IteratorRandom::choose` here to
        // round-robin across multiple connections to the same peer.
        // We open at most one connection per peer (PeerSession model),
        // so just take the first match. Avoids a `rand` dependency.
