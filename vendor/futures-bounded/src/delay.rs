...
impl Delay {
    ...
    pub fn tokio(duration: Duration) -> Self {
        // On wasm32 the real tokio timer panics ("time not implemented on this
        // platform" — std::time::Instant::now is unsupported), so fall back to
        // futures-timer, which uses browser timers (gloo-timers) on wasm.
